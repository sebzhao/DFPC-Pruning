...
parser.add_argument("--dataset", help="dataset name", choices=["imagenet", "cifar10", "cifar100"])
parser.add_argument(
    "-a",
    "--arch",
    metavar="ARCH",
...
parser.add_argument("--gamma", default=0.1, type=float, help="learning rate decay factor", dest="gamma")
parser.add_argument(
    "--decay-step-size",
    default=5,
    type=int,
    help="number of epochs after which learning rate is reduced",
    dest="decay_step_size",
)
parser.add_argument(
    "--accuracy-threshold",
    default=2.5,
    type=float,
    help="validation accuracy drop feasible for the pruned model",
    dest="accuracy_threshold",
)
parser.add_argument(
    "--accuracy-drop-to-stop",
    default=1,
    type=float,
...
parser.add_argument(
    ...
    default=0.01,
    type=float,
    help="percentage of channels to prune per pruning iteration",
    dest="pruning_percentage",
)
parser.add_argument(
    "--num-processes",
    default=5,
    type=int,  # More the merrier, but RAM consumption will increase drastically.
    help="number of simultaneous process to spawn for multiprocessing",
    dest="num_processors",
)
parser.add_argument(
    "--scoring-strategy",
    default="dfpc",
    type=str,
    help="strategy to compute saliencies of channels",
    dest="strategy",
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="prune coupled channels is set to 1",
    dest="prunecoupled",
    choices=[0, 1],
)
...
def main_worker(gpu, args):
    ...
    if args.dataset in ["cifar100"]:
        ...
        transform_train = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    (0.5070751592371323, 0.48654887331495095, 0.4409178433670343),  # mean
        ...
        transform_train = transforms.Compose(
            [
                ...
                ),  # these mean and var are from official PyTorch ImageNet example
            ]
        )
        transform_test = transforms.Compose(
        ...
        train_set = CIFAR10(data_path, train=True, download=True, transform=transform_train)
        test_set = CIFAR10(data_path, train=False, download=True, transform=transform_test)
        ...
        val_loader = torch.utils.data.DataLoader(
            test_set, batch_size=args.batch_size, num_workers=args.workers, shuffle=False, pin_memory=True
        )
    else:
        "Does nothing"
    ...
    if args.resume:
        ...
        if args.dataset in ["cifar100"]:
            ...
            base_model = torch.load("./transfer_model.pt")
        else:
            raise ValueError("Specified model architecture is not supported.")
    net = model = copy.deepcopy(base_model)
    macs, params = get_model_complexity_info(net, (3, 64, 64), as_strings=False, print_per_layer_stat=False)
    target_macs = macs * 0.57
    print("Target MACs: {}".format(target_macs))
    del net
    ...
    with open("saliency_scores.pkl", "wb") as f:
        pickle.dump(pruner, f)

    while macs > target_macs:  # FIXME: Replace this with macs threshold
        pruning_iteration += 1
        ...
        save_checkpoint(
            ...
            True,
            filename="dataparallel_model.pth.tar",
        )

        save_checkpoint({"arch": args.arch, "model": base_model}, True, filename="base_model.pth.tar")
